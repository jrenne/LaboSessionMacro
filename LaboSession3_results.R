...
data <- read.csv("~/Google Drive/Teaching/UNIL/Rcode/data/dataLaboSession3a.csv", sep="")
y <- data$V1
plot(y,type="l")
...
CI <- cbind(res.opt$par - 1.96*std.dev,res.opt$par + 1.96*std.dev)
...
print(min(matrix.BIC))
print(min(matrix.HQ))
fit.arima <- arima(y,order=c(8,0,3))
abs(eigen(make.F(fit.arima$coef[1:8]))$value)
